...
const API_PROVIDER_PRESETS: Record<ApiProviderType, ApiProviderPreset> = {
  ...
  antigravity: { base_url: "https://api.antigravity.ai/v1", models_path: "/models", auth_header: "Bearer" },
  gemini: { base_url: "https://generativelanguage.googleapis.com/v1beta", models_path: "/models", auth_header: "key" },
  minimax: { base_url: "https://api.minimax.chat/v1", models_path: "/models", auth_header: "Bearer" },
};
...
const OFFICIAL_API_PROVIDER_PRESETS = {
  ...
  "alibaba-coding-plan-openai": {
    ...
    api_key_hint: "Bailian Coding Plan keys for this preset must start with sk-sp- or sk-cp-.",
...
function buildModelsUrl(type: ApiProviderType, baseUrl: string, apiKey: string): string {
  ...
  if ((type === "google" || type === "gemini") && apiKey) {
...
export function registerApiProviderRoutes({ app, db, nowMs }: RegisterApiProviderRoutesOptions): void {
  ...
  app.post("/api/api-providers/:id/test", async (req, res) => {
    ...
    try {
      ...
      if (!resp.ok) {
        if (resp.status === 404 && officialPreset?.fallback_models?.length) {
          const models = mergeModelLists(officialPreset.fallback_models);
          const now = nowMs();
          ...
          return res.json({ ok: true, model_count: models.length, models });
        }
  ...
  app.get("/api/api-providers/:id/models", async (req, res) => {
    ...
    try {
      ...
      if (!resp.ok) {
        if (resp.status === 404 && officialPreset?.fallback_models?.length) {
          const models = mergeModelLists(officialPreset.fallback_models);
          return res.json({ ok: true, models, cached: false });
        }
        if (row.models_cache) {
