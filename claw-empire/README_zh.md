...
- **API 模型分配现在以 development 为默认起点，并扩展到已完成初始化的 office pack** - `development` 仍是默认基线，而已经 hydrate 到运行时状态的 office pack 也会一起出现在分配目标中。
...
- assign 弹窗会以 `development` 作为默认基线，同时把已完成初始设置并已 hydrate 的 office pack 一并显示出来。
