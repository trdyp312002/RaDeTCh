...
type StoredTransaction = { type: CashFlowKind; amount: number; account: string; currency: string }
...
export async function PUT(req: NextRequest, { params }: Params) {
  try {
    ...
    const oldResult = await db.execute({ sql: "SELECT type, amount, account, currency FROM cash_transactions WHERE id = ?", args: [id] })
    const old = oldResult.rows[0] as unknown as StoredTransaction | undefined
    if (!old) return NextResponse.json({ error: "ไม่พบรายการ" }, { status: 404 })
    await db.batch([
      ...balanceStatements(old, -1),
      { sql: "UPDATE cash_transactions SET type = ?, amount = ?, category = ?, account = ?, date = ?, note = ?, currency = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", args: [type, amount, category, account, date, note, currency, id] },
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  try {
    const { id } = await params; await db.execute(ensureFinanceItemsTable)
    const result = await db.execute({ sql: "SELECT type, amount, account, currency FROM cash_transactions WHERE id = ?", args: [id] })
    const old = result.rows[0] as unknown as StoredTransaction | undefined
    if (!old) return NextResponse.json({ error: "ไม่พบรายการ" }, { status: 404 })
    await db.batch([...balanceStatements(old, -1), { sql: "DELETE FROM cash_transactions WHERE id = ?", args: [id] }], "write")
